...
function Auction({title, auctionType, auction, data, height, customer}){
  ...
  const searchAuction= (searchVal) => {
    ...
    for(let i = 0; i<data.length; i++){
      if(data[i].assetName.toLowerCase().includes(searchVal.toLowerCase()) || data[i].nft_id.toLowerCase().includes(searchVal.toLowerCase()) || data[i].price_id.toLowerCase().includes(searchVal.toLowerCase())){
        newData.push(data[i])
      }
    }
  ...
  return(
      <>
        ...
        <div className = {styles.container}>
          ...
          <Carousel  ref={carousel} category={auction} auctionType={auctionType} data={dataList} height={height} customer={customer}/>:null}
